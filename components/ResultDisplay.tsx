...
import { Download, ArrowRight, Loader2, Palette, RefreshCcw, Share2, Sparkles, Brush, Camera } from 'lucide-react';
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 py-8 pb-32">
      ...
      <div className="flex flex-col md:flex-row items-center justify-center gap-8 mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="relative group shrink-0">
            ...
            <div className="relative bg-white p-2 shadow-lg rounded-lg transform rotate-[-3deg] w-40 md:w-64 transition-transform hover:rotate-0 duration-300">
                {originalImage && <img src={originalImage} alt="Original" className="w-full h-auto rounded" />}
                <p className="text-center text-xs text-gray-400 font-bold mt-2">元の写真</p>
            </div>
        </div>

        {/* Transition Arrow */}
        <div className="text-gray-300 shrink-0">
            {isGenerating ? <Sparkles className="animate-spin text-yellow-400" size={32}/> : <ArrowRight size={32} className="hidden md:block" />}
        ...
        <div className="relative w-full max-w-md shrink-0">
            <div className="bg-white p-3 md:p-4 shadow-2xl rounded-2xl w-full aspect-[3/4] flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 relative overflow-hidden">
                {generatedImage && !isGenerating ? (
                    <img src={generatedImage} alt="Generated Art" className="w-full h-full object-cover rounded-xl shadow-inner animate-in zoom-in duration-500" />
                ) : (
                     <div className="flex flex-col items-center justify-center text-gray-300 p-8 text-center w-full">
                        {isGenerating ? (
                            <div className="flex flex-col items-center gap-6 animate-in fade-in zoom-in duration-300">
                                <div className={`relative p-8 rounded-full ${currentLoading.bg} transition-colors duration-500`}>
                                  <div className={`relative z-10 ${currentLoading.color} transition-colors duration-500`}>
                                    {currentLoading.icon}
                                  </div>
                                ...
                                <span className={`font-bold text-xl ${currentLoading.color} animate-pulse transition-colors duration-500`}>
                                  {currentLoading.text}
                                </span>
                            </div>
                        ) : <span>写真を追加してね</span>}
      ...
      <div className="text-center mt-6 text-gray-400 font-medium text-sm">
        {generatedImage && !isGenerating ? "ボタンを押して違うスタイルも試してみてね！" : "写真を撮ってスタート！"}
      </div>
    </div>
  );
};
